...
const nextMatches = [
  { date: "2026-06-20", time: "09:00", group: "C", venue: "费城", match: "巴西 vs 海地" },
  { date: "2026-06-20", time: "12:00", group: "D", venue: "旧金山湾区", match: "土耳其 vs 巴拉圭" },
...
const dataStatus = [
  ...
  },
  {
    ...
    status: "已录入 30 场，待确认",
    need: "确认截至 2026-06-20 09:18 明确完赛的比分无误；直播中的巴西对海地未纳入",
    approved: false
  },
  {
    type: "results",
...
const confirmationNotes = [
  ...
  {
    title: "旧结果只作调试",
    body: "output 目录里的旧概率和图表已经标记为未确认调试输出，不能进入正式报告。"
  },
  {
    title: "R 主程序已加闸门",
  ...
  {
    title: "赛中数据单独处理",
    body: "截至 2026-06-20 09:18 的 30 场已赛结果只用于当前积分、滚动 Elo 和近 10 场状态，不回填到赛前训练数据。"
  },
  {
    title: "时间泄漏已修复",
...
function renderModelMetrics() {
  ...
  const metrics = [
    ...
    { label: "调试参数", value: "--allow-unconfirmed-data" }
  ];
...
function renderCoefChart() {
  ...
  const items = [
    ...
    "确认分组、赛程、场地和槽位",
    "确认截至 2026-06-20 09:18 的 30 场已结束比分与排名",
...
async function loadRouteDashboard() {
  ...
  try {
    ...
    state.textContent = routeDataset.formal
      ...
      : `调试预览：${routeDataset.simulations.toLocaleString()} 次模拟；当前使用 ${routeDataset.annexMode}，不得作为正式结论。`;
    renderOverallRoute();
    renderTeamRoute();
