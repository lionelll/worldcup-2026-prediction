...
check_data_approval <- function(path) {
  if (allow_unconfirmed_data) {
    warning("--allow-unconfirmed-data is enabled. Chart inputs have not completed source verification.", call. = FALSE)
  ...
  if (nrow(pending) > 0) {
    stop(sprintf(
      "Data source confirmation required before formal chart generation: %s. Use --allow-unconfirmed-data only to validate pipeline execution.",
...
preview_label <- ifelse(formal_run, "", "阶段性分析：最佳第三名分配映射尚未最终核验")
chart_family <- "sans"
