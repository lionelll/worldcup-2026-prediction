...
validate_annex <- function(path) {
  if (!file.exists(path)) {
    if (allow_unconfirmed_data) {
      warning("Annex C mapping is absent; validation continues in non-formal mode.", call. = FALSE)
...
cat(sprintf("Annex C: %s\n", ifelse(file.exists(annex_path), annex_path, "missing (non-formal validation only)")))
