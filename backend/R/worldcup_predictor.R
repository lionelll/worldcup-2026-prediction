...
check_data_approval <- function(path, required_files) {
  if (allow_unconfirmed_data) {
    warning("--allow-unconfirmed-data is enabled. Model inputs have not completed source verification.", call. = FALSE)
  ...
  if (nrow(pending) > 0) {
    stop(sprintf(
      "Data source confirmation required before formal modeling: %s. Review data/DATA_STATUS.md and set approved=true only after source verification. Use --allow-unconfirmed-data only to validate pipeline execution.",
...
load_annex_mapping <- function(path) {
  if (!file.exists(path)) {
    ...
    warning("Annex C mapping is absent. Non-formal validation will use candidate-slot matching and is not a formal prediction.", call. = FALSE)
    return(data.frame())
  }
  df <- read_csv_safe(path)
...
debug_third_assignment <- function(best_thirds, bracket) {
  ...
  if (is.null(assigned_groups) || length(assigned_groups) != 8) {
    stop("Candidate third-place slots cannot be matched one-to-one in non-formal validation mode.", call. = FALSE)
  }
