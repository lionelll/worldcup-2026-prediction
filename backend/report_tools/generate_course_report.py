#!/usr/bin/env python3
"""Build the analysis DOCX from the current R model outputs.

The modelling and simulation code remains in R/. This script assembles its
reproducible outputs into a Word document. Formal generation is blocked until
every required data source is approved; --allow-unconfirmed-data creates a
clearly labelled non-formal report.
"""
...
def build_abstract(doc, metadata, metrics, observed, draft):
    ...
    text = (
        ...
        f"验证集共 {int(float(metrics['samples']))} 场，胜平负准确率为 {accuracy:.2%}，"
        f"Brier Score 为 {float(metrics['brier_score']):.4f}，Log Loss 为 {float(metrics['log_loss']):.4f}。"
        f"在截至 2026 年 6 月 20 日录入的 {int(float(observed['matches']))} 场世界杯赛果上，"
        f"方向命中率为 {obs_accuracy:.2%}。赛中样本出现 {int(float(observed['actual_draws']))} 场平局，"
        "而最高概率类别没有选择平局，说明当前模型的主要误差来自平局与低比分相关结构。"
        "本报告为基于截至 2026 年 6 月 20 日比赛快照的阶段性分析。本文同时给出冠军概率、各阶段晋级概率、"
        "赛中预测偏差及模型风险；数据来源与最佳第三名分配映射的限制集中在结论和附录说明。"
    )
    add_text(doc, text)
    p = doc.add_paragraph()
    ...
    if draft:
        add_callout(
            doc,
            "报告状态声明",
            "本报告使用尚未完成来源核验的数据快照。完成全部关键数据核验后，应重新运行 R 程序并生成正式结果。",
            fill=CAUTION,
        )
    add_page_break(doc)
...
def build_report(doc, *, draft: bool):
    ...
    snapshots = read_csv(OUTPUT / "snapshot_change_2026-06-20_30_to_32.csv")
    ...
    questions = [
        ...
        "纳入截至 2026 年 6 月 20 日明确完赛的 32 场结果后，各队后续晋级和夺冠概率如何变化？",
    ...
    add_text(doc, "历史模型的时间边界固定为 2026 年 6 月 10 日。2026 年 6 月 11 日之后的世界杯赛果不进入训练集，只用于更新当前小组积分、Elo 和近期状态，并用于检验模型与现实比赛的差异。这样既保留了赛前模型的可验证性，也允许报告给出截至 6 月 20 日共 32 场完赛后的动态预测。直播中或未明确完赛的比分不纳入。")
    ...
    data_rows = [
        ...
        ["模型输出", "output/*.csv", f"{int(float(metadata['simulations'])):,} 次模拟", "当前快照阶段性输出"],
    ]
    ...
    split_rows = [
        ...
        ["赛中观测集", "2026-06-12 至 2026-06-20", str(len(results)), "不回填训练，仅更新当前状态"],
    ]
    ...
    sims = int(float(metadata["simulations"]))
    add_text(doc, f"从截至 6 月 20 日共 32 场完赛后的状态出发，剩余赛程按泊松概率抽样，完整赛事重复 {sims:,} 次。每次模拟记录各队到达的最远轮次，以频率估计 32 强、16 强、8 强、四强、决赛和夺冠概率。随机种子固定为 {metadata['seed']}。")
    ...
    risks = [
        ["最佳第三名分配映射", "当前没有 FIFA Annex C 的 495 种完整组合映射；程序使用官方赛程候选槽位约束进行近似分配。", "高"],
        ...
        ["模拟次数", f"当前为 {sims:,} 次，已达到课程报告的模拟稳定性建议下限；结果按阶段性快照口径解释。", "中"],
    ]
    ...
    add_text(doc, "验证集中 77 场实际平局没有一场被选为最高概率类别。概率模型仍给出平均 23.55% 的平局概率，因此 Brier Score 与 Log Loss 仍有意义；但在输出离散胜平负类别时，‘不预测平局’仍是必须修正的明显缺陷。")
    ...
    add_figure(doc, "fig5_5_logistic_roc.png", "图 5-6　二元逻辑回归 ROC 曲线", width_cm=10.5)
    ...
    add_heading(doc, "第六章　截至 2026-06-20 的赛中检验", 1)
    ...
    add_text(doc, f"本次由 30 场快照新增录入第 31 至 32 场明确完赛比赛，模型方向命中 {latest_hits}/{len(latest_results)}：{'；'.join(latest_descriptions)}。两场赛果均已进入当前积分、Elo 与近期状态更新。")
    add_page_break(doc)

    add_heading(doc, "第七章　世界杯模拟结果", 1)
    add_callout(doc, "结果口径", f"本章读取 {sims:,} 次、随机种子 {metadata['seed']} 的当前快照模拟。置信区间基于二项频率标准误，不是 bootstrap。最佳第三名分配映射与数据来源尚未完成最终核验，因此路线结果用于比较当前模型内部的相对排序。", fill=CAUTION)
    ...
    add_text(doc, f"当前模拟中，{champion[0]['team']}夺冠概率为 {float(champion[0]['probability']):.1%}，{champion[1]['team']}为 {float(champion[1]['probability']):.1%}。两队区间明显重叠，不能把 {lead_gap * 100:.1f} 个百分点的差异解释为稳定领先。第三名之后概率快速下降，说明当前模型将冠军概率高度集中在前两队；这一集中程度仍需在补齐最佳第三名分配映射、修正东道主变量并使用不同随机种子重复运行后核验。")
    ...
    annex_label = "FIFA 官方完整映射" if annex_mode == "official" else "官方赛程候选槽位约束近似"
    add_callout(
        doc,
        "路线图读取边界",
        f"本次路线输出使用{annex_label}。图中‘遇’为条件遇敌概率，‘胜’为该对阵中的模拟晋级率，‘贡献’为该分支对下一轮无条件概率的贡献。遇敌样本少于 30 次时标记为样本不足。该近似对路线概率的影响在第九章集中说明。",
        fill=CAUTION,
    )
    ...
    add_heading(doc, "8.1 第 31 至 32 场赛果带来的概率变化", 2)
    ...
    add_text(doc, f"本次 32 场快照相对 30 场快照新增巴西 3:0 海地、土耳其 0:1 巴拉圭两场结果。32 强概率上升最多的是{top_gain['team']}（{float(top_gain['round_of_32_change']):+.1%}），下降最多的是{top_loss['team']}（{float(top_loss['round_of_32_change']):+.1%}）。概率变化同时来自已赛积分、净胜球、动态 Elo 与滚动状态更新，不能只归因于单一因素。")
    ...
    conclusions = [
        ...
        f"截至 6 月 20 日的 {observed_matches} 场赛中命中率为 {float(observed['accuracy']):.2%}；{int(float(observed['actual_draws']))} 场实际平局中，最高概率类别预测平局 {int(float(observed['predicted_draws']))} 场。",
        f"当前快照模拟中，{champion[0]['team']}与{champion[1]['team']}位列冠军概率前二，但区间重叠，不能宣称第一名具有统计上稳定优势。",
        "已赛积分和净胜球能显著改变小组出线概率，赛中更新对冠军概率的影响相对较小。",
        "当前最优先的改进不是继续增加图表，而是确认数据、实现完整最佳第三名分配映射、修正东道主变量并改善低比分相关模型。",
    ]
    ...
    improvements = [
        ...
        ["P0", "最佳第三名分配映射", "录入 FIFA Annex C 的 495 种最佳第三名组合到淘汰赛槽位的完整映射，并增加自动测试。"],
    ...
    refs = [
        ...
        "[11] FIFA. FIFA World Cup 26 official tournament page. https://www.fifa.com/en/tournaments/mens/worldcup/canadamexicousa2026.",
        "[12] 百度体育. 世界杯赛程与排名页. https://tiyu.baidu.com/al/match?match=世界杯&tab=赛程&current=0.",
    ...
    code_rows = [
        ...
        ["R/README.md", "运行顺序、数据确认闸门与阶段性运行参数说明"],
    ]
    add_caption(doc, "表 A-1　主要代码文件")
    add_table(doc, ["文件", "作用"], code_rows, [3000, CONTENT_WIDTH_DXA-3000], font_size=9.2)
    add_text(doc, "以下命令复现当前数据快照。完成全部数据来源核验并补齐 Annex C 映射后，将 validate 替换为 formal。")
    code = (
    ...
    for line in code.splitlines():
        ...
        r.add_break()
    add_text(doc, "报告中的概率、评价指标和图表均从 backend/output 目录读取。源代码、输入数据和运行资源按上述目录组织，结果可由统一脚本复现。")
    ...
    check_rows = [
        ...
        ["关键数据来源核验", "原始来源与快照日期尚待补齐", "限制项"],
        ["最佳第三名分配映射", "采用官方赛程候选槽位约束近似", "限制项"],
    ]
    ...
    add_text(doc, "网页通过浏览器读取 frontend/data 中的 CSV，因此不能直接双击 index.html 打开；应在仓库根目录启动本地静态服务器。", indent=False)
    web_commands = (
...
def main():
    ...
    if not formal and not args.allow_unconfirmed_data:
        raise SystemExit(
            "Data confirmation is incomplete. Use --allow-unconfirmed-data only to create a clearly labelled non-formal report."
        )
    draft = not formal
